...
cover_frame = tk.LabelFrame(main_frame, text="Cover Image (For Encryption)", bg="lightgreen")
cover_frame.grid(row=0, column=0, padx=10, pady=5, sticky="ew")
btn_load_cover = tk.Button(cover_frame, text="Load Cover Image", command=load_cover_image, bg="white")
btn_load_cover.grid(row=0, column=0, padx=5, pady=5)
cover_label = tk.Label(cover_frame, text="No file selected", bg="lightgreen")
cover_label.grid(row=0, column=1, padx=5, pady=5)

# --- Encryption Section ---
enc_frame = tk.LabelFrame(main_frame, text="Encryption", bg="lightyellow")
enc_frame.grid(row=1, column=0, padx=10, pady=5, sticky="ew")
tk.Label(enc_frame, text="Secret Message:", bg="lightyellow").grid(row=0, column=0, padx=5, pady=5, sticky="e")
enc_message_entry = tk.Entry(enc_frame, width=40)
enc_message_entry.grid(row=0, column=1, padx=5, pady=5)
tk.Label(enc_frame, text="Passcode:", bg="lightyellow").grid(row=1, column=0, padx=5, pady=5, sticky="e")
enc_password_entry = tk.Entry(enc_frame, width=40, show="*")
enc_password_entry.grid(row=1, column=1, padx=5, pady=5)
btn_encrypt = tk.Button(enc_frame, text="Encrypt", command=encrypt_message, bg="white")
btn_encrypt.grid(row=2, column=0, columnspan=2, pady=5)

# --- Encrypted Image Section (For Decryption) ---
encrypted_frame = tk.LabelFrame(main_frame, text="Encrypted Image (For Decryption)", bg="lightcoral")
encrypted_frame.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
btn_load_encrypted = tk.Button(encrypted_frame, text="Load Encrypted Image", command=load_encrypted_image, bg="white")
btn_load_encrypted.grid(row=0, column=0, padx=5, pady=5)
encrypted_label = tk.Label(encrypted_frame, text="No file selected", bg="lightcoral")
encrypted_label.grid(row=0, column=1, padx=5, pady=5)

# --- Decryption Section ---
dec_frame = tk.LabelFrame(main_frame, text="Decryption", bg="lightcyan")
dec_frame.grid(row=3, column=0, padx=10, pady=5, sticky="ew")
tk.Label(dec_frame, text="Passcode:", bg="lightcyan").grid(row=0, column=0, padx=5, pady=5, sticky="e")
dec_password_entry = tk.Entry(dec_frame, width=40, show="*")
dec_password_entry.grid(row=0, column=1, padx=5, pady=5)
tk.Label(dec_frame, text="Message Length:", bg="lightcyan").grid(row=1, column=0, padx=5, pady=5, sticky="e")
dec_length_entry = tk.Entry(dec_frame, width=40)
dec_length_entry.grid(row=1, column=1, padx=5, pady=5)
btn_decrypt = tk.Button(dec_frame, text="Decrypt", command=decrypt_message, bg="white")
btn_decrypt.grid(row=2, column=0, columnspan=2, pady=5)
tk.Label(dec_frame, text="Decrypted Message:", bg="lightcyan").grid(row=3, column=0, padx=5, pady=5, sticky="ne")
